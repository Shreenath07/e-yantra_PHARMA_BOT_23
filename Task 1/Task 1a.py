...
#Detect color 
"""The line all(img[5][5] == color) is used in OpenCV (Open Source Computer Vision Library) 
   to check if the pixel at coordinates (5, 5) in the image img matches the specified color."""
def checkcolor(img,color):
   return all(img[5][5] == color) 


#To find the location of shops in the arena
def find_shops(img):
    shop=dict()
    for i in range(6):
...
# shape detetion with the contour values as input
